...
func _test_hud_contract() -> void:
	...
	_check("12 K" in hud.landing_stats.text and "3–2" in hud.landing_stats.text, "Landing must display saved records")
	_check("VERSUS" in hud.landing_prompt.text, "Landing CTA stitch must follow focused mode")
